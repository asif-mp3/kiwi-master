...
from planning_layer.table_router import TableRouter  # NEW: LLM-based table selection
...
# Initialize table router (uses LLM-based semantic selection)
_table_router = None
...
def run(question: str):
    ...
    # 1. NEW: Use intelligent table routing with LLM-based selection
    # This replaces the old vector-based schema retrieval for better accuracy
    global _table_router
    if _table_router is None:
        _table_router = TableRouter()

    # Route to the best table using hybrid LLM selection
    routing_result = _table_router.route(question)

    print(f"\n🎯 TABLE ROUTING:")
    print(f"   Selected: {routing_result.table}")
    print(f"   Confidence: {routing_result.confidence:.0%}")
    print(f"   Method: {_table_router.get_routing_debug().get('method', 'unknown')}")
    if _table_router.get_routing_debug().get('reason'):
        print(f"   Reason: {_table_router.get_routing_debug().get('reason')}")

    # Get schema context for the selected table
    if routing_result.table and routing_result.confidence >= 0.5:
        # Use focused schema from table router
        schema_context = _table_router.get_table_schema(routing_result.table)
        print(f"\nFOCUSED SCHEMA CONTEXT (single table):")
        print(schema_context[:500] + "..." if len(schema_context) > 500 else schema_context)
    else:
        # Fallback to vector-based retrieval if routing failed
        print("\n⚠️  Low confidence routing, falling back to vector retrieval...")
        schema_context = retrieve_schema(question)
        print("\nRETRIEVED SCHEMA CONTEXT (vector search):")
        for item in schema_context:
            print("-", item["text"])

    # 2. Planning with entity hints
    plan = generate_plan(question, schema_context, entities=routing_result.entities)
    validate_plan(plan)
    ...
    if result.empty and plan["query_type"] in ["lookup", "filter"]:
        ...
        # Get alternative tables from routing result (LLM-based) or schema context
        alternative_tables = []
        if routing_result.alternatives:
            # Use alternatives from LLM-based routing
            alternative_tables = [t for t, _ in routing_result.alternatives if t != plan["table"]]
        else:
            # Fallback to schema context
            for item in schema_context if isinstance(schema_context, list) else []:
