...
SHEET_REGISTRY_FILE = "backend/data_sources/snapshots/sheet_state.json"
...
def clear_sheet_state():
    """
    Clear sheet state to force full rebuild on next connect.
    Used for session reset.
    """
    try:
        if Path(SHEET_REGISTRY_FILE).exists():
            Path(SHEET_REGISTRY_FILE).unlink()
            print("✓ Sheet state cleared - next connect will perform full rebuild")
    except Exception as e:
        print(f"⚠️  Could not clear sheet state: {e}")
